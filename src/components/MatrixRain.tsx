...
export default memo(function MatrixRain() {
  ...
  const actualOpacity = matrixOpacity > 0 && effectsMode !== "none" ? matrixOpacity : 0
  ...
  const createDrawFunction = useCallback(() => {
    ...
    const fadeOpacity = isMobileRef.current ? CONFIG.mobileFadeOpacity : CONFIG.desktopFadeOpacity
    const dropSpeed = isMobileRef.current ? CONFIG.dropSpeed.mobile : CONFIG.dropSpeed.desktop
  ...
  const handleResize = useCallback(() => {
    ...
    if (newColumns > dropsRef.current.length) {
      // Expand drops array
      const additionalDrops = Array(newColumns - dropsRef.current.length).fill(1)
  ...
  useEffect(() => {
    ...
    const frameDelay = isMobileRef.current ? CONFIG.mobileFrameDelay : CONFIG.desktopFrameDelay
    let lastFrameTime = 0
  ...
  }, [actualOpacity, effectsMode, isLoadingComplete, createDrawFunction, handleResize])
